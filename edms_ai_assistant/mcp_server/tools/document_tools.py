# mcp-server/tools/document_tools.py
"""
Инструменты работы с документами для FastMCP.
Перенесены из edms_ai_assistant/tools/document.py, document_versions.py,
document_comparison.py, doc_search.py.
"""
...
from ..clients.document_client import DocumentClient, FULL_DOC_INCLUDES
...
def _att_name(attachment: Any) -> str:
    if isinstance(attachment, dict):
        return (
            attachment.get("name") or attachment.get("originalName")
            or attachment.get("fileName") or ""
        )
    return ""
...
def register_document_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool(
        description=(
            "Получить полный анализ документа СЭД: метаданные, участников, вложения, "
            "поручения, контроль, ознакомления. Используй когда нужна детальная информация "
    ...
    async def doc_get_details(
        ...
        try:
            ...
            logger.error("doc_get_details failed for %s: %s", document_id, exc, exc_info=True)
            return {"status": "error", "error": f"Ошибка обработки документа: {exc}"}

    @mcp.tool(
        description=(
            "Получить все версии документа и автоматически сравнить каждую пару соседних версий. "
            "Возвращает полную историю изменений метаданных и вложений. "
            "НЕ нужно спрашивать пользователя какие версии сравнивать — сравниваются все."
        )
    )
    async def doc_get_versions(
        ...
        try:
                ...
                for i in range(len(version_nums) - 1):
                    ...
                    try:
                        ...
                        if not doc_from or not doc_to:
                            errors.append(f"Версия {from_vnum} или {to_vnum}: метаданные недоступны")
                            continue
                        ...
                        errors.append(f"Ошибка сравнения {from_vnum}↔{to_vnum}: {pair_exc}")
            ...
            return {
                ...
                "message": (
                    f"Документ имеет {total} версии. Выполнено {len(comparisons)} сравнений. "
                    + ("Изменения обнаружены." if has_changes else "Версии идентичны.")
                ),
                "instruction": "Все сравнения выполнены. Сформулируй ответ на основе поля 'comparisons'.",
            }
    ...
    @mcp.tool(
        description=(
            ...
            "Используй когда нужно найти различия между двумя конкретными документами."
        )
    )
    async def doc_compare_documents(
        ...
        try:
            ...
            if not doc1 or not doc2:
                return {"status": "error", "message": "Один или оба документа не найдены."}
    ...
    @mcp.tool(
        description=(
            "Поиск документов в EDMS по фильтрам: краткое содержание, рег. номер, "
            "категория, статус, даты, автор, корреспондент, исполнитель поручения, "
            "флаги текущего пользователя. Возвращает до 10 документов."
        )
    )
    async def doc_search_tool(
        ...
        Args:
            ...
            reg_number: Рег. номер (входящий).
            out_reg_number: Исходящий рег. номер.
            doc_category: INCOMING|OUTGOING|INTERN|APPEAL|CONTRACT|MEETING|QUESTION|CUSTOM.
            status: Список статусов: FORMING|REGISTRATION|IN_PROGRESS|COMPLETED|CANCELLED...
            date_from: Дата регистрации от (YYYY-MM-DD).
            date_to: Дата регистрации до (YYYY-MM-DD).
            author_last_name: Фамилия автора.
            correspondent_name: Название организации-корреспондента.
            author_current_user: True — только мои документы.
            process_executor_current_user: True — документы на моём рассмотрении.
            task_executor_current_user: True — мои поручения.
            control_user_current_user: True — документы на моём контроле.
            introduction_current_user: True — документы на ознакомлении у меня.
        """
        from datetime import datetime as dt

        def to_iso_start(date_str: str) -> str:
            d = dt.fromisoformat(date_str)
            return d.replace(hour=0, minute=0, second=0).strftime("%Y-%m-%dT%H:%M:%S.000Z")

        def to_iso_end(date_str: str) -> str:
            d = dt.fromisoformat(date_str)
            return d.replace(hour=23, minute=59, second=59).strftime("%Y-%m-%dT%H:%M:%S.999Z")
        ...
        if date_control_start:
            doc_filter["dateControlStart"] = to_iso_start(date_control_start)
        if date_control_end:
            doc_filter["dateControlEnd"] = to_iso_end(date_control_end)
        if author_last_name:
...
def _format_author(author: dict[str, Any] | None) -> str:
    if not author:
        return "—"
    parts = [author.get("lastName", ""), author.get("firstName", ""), author.get("middleName", "")]
